...
matches_controlled = matches / swipes
matches_controlled = matches_controlled.rolling(window=10, min_periods = 1).mean()

#plotting result
my_dpi=96
plt.figure(figsize=(1000/my_dpi, 300/my_dpi), dpi=my_dpi)
plt.plot(left_vs_right.index[5:], left_vs_right.iloc[5:,0], color='lightblue',  marker='', label='Pickiness: left vs right swipe ratio', linewidth=2)
plt.plot(matches_controlled.index[5:], matches_controlled.iloc[5:,0] ,marker='', color='orange', label = 'Success: Likelihood of match on swipe', linewidth=2)
plt.title("Tinder pickiness versus success",fontdict={'family': 'serif',
        'weight': 'bold',
        'size': 10,
        })
...
left_vs_right.corrwith(matches_controlled)